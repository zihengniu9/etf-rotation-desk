class FinancialComposite(Factor):
    def calculate(self, factors):
        ...
        growth = (
            0.25 * RANK(MAX(-50.0, MIN(200.0, rev_growth)))
            + 0.35 * RANK(MAX(-80.0, MIN(500.0, profit_growth)))
            + 0.15 * RANK(MAX(-100.0, MIN(150.0, rev_growth - prev_rev_growth)))
            + 0.15 * RANK(MAX(-300.0, MIN(500.0, profit_growth - prev_profit_growth)))
            + 0.10 * persistence
        )
        ...
        score = 0.50 * growth + (1.0 / 3.0) * quality + (1.0 / 6.0) * balance
